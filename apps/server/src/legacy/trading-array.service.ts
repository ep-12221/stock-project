...
import type { Order, OrderBook, Trade } from '../domain/models.js';
import type { MemoryStore } from '../store/memory-store.js';
import { integer, invariant } from '../matching/checked.js';
import { compareOrders, isActive, planMatches } from '../matching/order-book.js';
import {
  assertConservation,
  assertOrder,
  assertReservations,
...
export function createTradingService(store: MemoryStore, options: TradingOptions = {}) {
  ...
  function executeOrder(userId: string, input: unknown, requireKey: boolean): SubmitOrderResult {
    ...
    const originalBook = store.orderBooks.get(data.symbol) ?? { buyOrderIds: [], sellOrderIds: [] };
    const storedOrder = (id: string): Order => {
      const order = store.orders.get(id);
      invariant(order);
      return order;
    };
    const allBookIds = [...originalBook.buyOrderIds, ...originalBook.sellOrderIds];
    invariant(new Set(allBookIds).size === allBookIds.length);
    for (const [ids, side] of [
      [originalBook.buyOrderIds, 'BUY'],
      [originalBook.sellOrderIds, 'SELL'],
    ] as const) {
      for (const [index, id] of ids.entries()) {
        const order = storedOrder(id);
        assertOrder(order);
        invariant(isActive(order) && order.symbol === data.symbol && order.side === side);
        invariant(store.activeOrdersByUser.get(order.userId)?.has(id));
        if (index > 0) invariant(compareOrders(side, storedOrder(ids[index - 1]!), order) < 0);
      }
    }
    const incoming = { ...data, userId };
    const makers = (data.side === 'BUY' ? originalBook.sellOrderIds : originalBook.buyOrderIds).map(
      storedOrder,
    );
    const plan = planMatches(incoming, makers);
    ...
    const book: OrderBook = {
      buyOrderIds: originalBook.buyOrderIds.filter((id) => isActive(getOrder(id))),
      sellOrderIds: originalBook.sellOrderIds.filter((id) => isActive(getOrder(id))),
    };
    if (isActive(order))
      (order.side === 'BUY' ? book.buyOrderIds : book.sellOrderIds).push(order.id);
    book.buyOrderIds.sort((a, b) => compareOrders('BUY', getOrder(a), getOrder(b)));
    book.sellOrderIds.sort((a, b) => compareOrders('SELL', getOrder(a), getOrder(b)));
    const bestBidCents = book.buyOrderIds[0] ? getOrder(book.buyOrderIds[0]).priceCents : null;
    const bestAskCents = book.sellOrderIds[0] ? getOrder(book.sellOrderIds[0]).priceCents : null;
    ...
    // Commit only after every calculation and check. No await, callback or external I/O in this command.
    ...
    store.orderBooks.set(data.symbol, book);
