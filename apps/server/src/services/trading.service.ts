...
import { createTradingService as createLegacyTradingService } from '../legacy/trading-array.service.js';
...
/** One current command path; the preserved legacy engine is only selected for A/B comparison. */
export function createTradingService(store: MemoryStore, options: TradingOptions = {}) {
  if (store.matchingEngine === 'array') return createLegacyTradingService(store, options);
